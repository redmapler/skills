...
1. [敏感词完整清单（115 项）](#1-敏感词完整清单115-项)
2. [风险词处理原则](#2-风险词处理原则)
...
- **场景例外**：极少数词在"非功能声明、纯客观描述材质/品类"时可能可用，但**默认禁止**，需人工复核并在 `qa_report.md` 的合规检查记录中记录豁免理由。
...
- **A 级（强证据，可表达）**：第三方验证、官方测试报告、认证证书等。
- **B 级（产品事实）**：从底图可验证的实物证据（结构、颜色、材质、可数细节等），可用于客观事实描述。
- **C 级（卖家说明，仅可弱化表达）**：卖家口头或文字声明，无第三方验证。仅可使用"设计用于…"等弱化措辞，且需在聊天中向用户确认待补证据。
- **D 级（模型推测，禁止表达）**：AI 基于常识的推测，无任何证据。
...
| 等级 | 定义 | 可声明范围 |
|------|------|------------|
| A 级 | 第三方验证 / 官方测试报告 / 有效认证证书 | 可表达对应功能声明，使用第 5 节替代表达 |
| B 级 | 从底图可验证的实物证据（结构、颜色、材质、可数细节等） | 可用于客观事实描述 |
| C 级 | 卖家口头或文字声明，无第三方验证 | 仅可用"designed to""help"等弱化措辞，并标注待补证据 |
| D 级 | AI 基于常识的推测，无任何证据 | 禁止任何功能声明 |
...
1. **确认产品真实性能**：从 `approved_image_plan.json` 中的 `product_truth` 核对产品事实。
2. **确认证据等级**：查第 4 节，确认达 A 级。
3. **选择替代表达**：从上表选择，确保措辞与证据数据一致。
4. **记录留痕**：在 `qa_report.md` 的合规检查记录中记录"原词 -> 替代表达 -> 证据来源"。
...
5. **记录输出**：将所有处理结果写入 `qa_report.md` 的合规检查记录，格式如下：
