...
> 3. 合规优先 —— 所有 Prompt 不得出现 `references/claims_compliance.md` 中列出的 117 个敏感词。
...
- **字数**：约 200 字。
- **数据来源**：从 `01_product_truth_summary.md` 中已锁定的产品事实提取。
...
- **核心规则**：参考 `references/claims_compliance.md` 中的 117 个敏感词，
  确保不出现任何违规表达。
...
### 8.2 Desktop Prompt 模板（1464 × 600）

A+ 桌面版图片为横向宽幅 banner 格式，采用左右分区构图，适合展示场景全景和较丰富的信息。
...
- 构图：left-right split composition
  - 左侧（约 55%）：窗帘悬挂在窗前的客厅场景，自然光透过窗帘，
    展示垂坠质感和遮光效果
  - 右侧（约 45%）：深灰渐变背景上的信息区
- 文字标注1：大号白色粗体 "100% BLACKOUT"，位于右上方
...
- 构图：top-bottom split composition
  ...
  - 下方（约 40%）：深灰渐变背景信息区
- 文字标注1：特大号白色粗体 "100% BLACKOUT"，位于下方居中
...
| 色块使用 | 可左右渐变 | 可上下渐变 |
| 尺寸比例 | 1464 × 600（约 2.44:1） | 1600 × 1200（4:3） |
| 文字标注数量 | 2-3 个 | 最多 2 个 |
...
2. **建议使用短词和数字**：尽量使用不超过 3 个单词的短词组合和纯数字标注，
   如 "100% BLACKOUT"、"2 PANELS"、"52" × 108""。避免长句和复杂专有名词。
